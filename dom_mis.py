...
def generate_graph_from_similarity_matrix(min_similarity_threshold, na_handling_setting_name='dropna'):
    ela_representation_df = pd.read_csv(f'data/aggregated_ela_representation_{na_handling_setting_name}.csv',
                                        index_col=[0, 1, 2])
...
def run_dom_mis(run_id, similarity_thresholds, save_results=False):
    ...
    for na_handling_setting_name in ['dropna']:
        ...
        for min_similarity_threshold in similarity_thresholds:
            ...
            for algorithm_name, algorithm_results in [('dominant', dominating_set(g)),
